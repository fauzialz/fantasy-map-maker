...
| **E5** | Does the live-layer rule change? | **Yes** — active *or* holds a selection. Rejected leaving it, which re-caches a bitmap per layer per drag frame. **Measure it in WP-18**; the earlier worry that this re-opens ADR-19's memory trap was wrong in direction — live layers hold no bitmap. |
