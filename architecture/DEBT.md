...
| Deploying the built SPA | WP-13's entry, and now a **multi-file** deploy: `dist/` holds four HTML entries and the host must route `/maps*` to `app.html` (ADR-40, `README.md`, and the Caddyfile in `platform/01-zitadel-setup.md` §4). **The host is now chosen** — the VPS, behind Caddy, frontend-only (ADR-34, `platform/README.md` D4). Unbuilt work, not debt |
