...
| Open design decisions | **None with an owner** — every decision belonging to a scheduled package is settled: `v1/08` D1–D6, `v1/09` E1–E15, `v1/10` F1–F5, `v1/12` D1–D4, `v1/13` D5–D10, `v1/14` D1–D12. **Five are open with no owner: `v1/15` N1–N5**, the river-network questions (one object or many · derived width as invariant or default · does the coast stroke break at a mouth · do lakes participate · does the generator emit networks). They are **row V-02 above** rather than a pointer, because nothing is scheduled to answer them and a pointer to an unscheduled note is how a question gets lost |
| Features deferred from v1 | `v1/01-system-design.md` §15. Absence of a feature is not debt |
| Unbuilt work packages | **None at all** — every 0.5 package through WP-38 has shipped. `v1/05-p0-build-checklist.md` tracks P1–P3 |
| Deploying the built SPA | WP-13's entry, and now a **multi-file** deploy: `dist/` holds four HTML entries and the host must route `/maps*` to `app.html` (ADR-40, `README.md`, and the nginx site in `platform/01-zitadel-setup.md` §4). **The host is now chosen** — the VPS, behind **nginx** (ADR-46, amending ADR-34 and `platform/README.md` D4), frontend-only. Unbuilt work, not debt |
